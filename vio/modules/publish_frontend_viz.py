...
OPT-IN (``--frontend-viz``). The default :class:`~vio.modules.pipeline.OdometryModule`
does NOT register this step (it builds the plain ``KLTFrontend`` and leaves
``frontend_viz`` off), so the deterministic / oracle path never captures this and
the returned tracks stay byte-identical -- the byte-parity oracle is UNAFFECTED.

When enabled, the odometry module builds a
:class:`sky.front.frontend.CaptureKLTFrontend` (instead of the plain
``KLTFrontend``) which, as a pure side effect of
:meth:`~vio.modules.track_features.TrackFeatures`, stashes one
:class:`sky.front.frontend.FrontendVizSnap` per frame on its side-car. This step
is chained right AFTER :class:`~vio.modules.publish_tracks.PublishTracks` (so the
...
class PublishFrontendViz(Step):
    name = "publish_frontend_viz"

    def run(self, ctx, tracked: Tracked):
        vo = ctx.state["vo"]
        fe = vo.frontend
        # Only the capture frontend stashes a snap; anything else is a no-op
        # (defensive -- the module only wires this step with the capture frontend).
        ...
        ctx.bus.publish(topics.FRAME_FRONTEND, FrameFrontend(
