...
triplet dump. It reuses the exact design language the user already approved in
the imu/cam window -- themed :class:`QFrame` panels, the auto-scaling
:class:`~ui.qt.imu_panels.GyroPlot` and the interactive 3D
:class:`~ui.qt.imu_panels.Accel3DView` -- and adds a real **depth** panel between
the camera image and the IMU.
...
The data is **subscribed from the running acquisition front-end**. The window
builds a private :class:`~ui.comms.LocalPubSub` (no odometry/backend/slam) and a
:class:`~ui.modules.triplet.UiTripletModule` sink joins ``frame.depth`` (the
rectified-left image + metric depth the depth step emits) with ``imucam.sample``
(the calibrated IMU rows for that frame) by ``seq``. No second device, no second
SGM: the triplet is literally the pipeline's output.

Worker model (injected ``worker_factory``)
-----------------------------------------
The window drives the identical view through an injected zero-arg worker factory.
In the 4-process proc4 UI the factory is ALWAYS the IPC adapter
(:func:`ui.modules.ipc_sources.ipc_triplet_factory`), which republishes capture's
``imucam.sample`` + ``frame.depth`` over IPC -- the UI never opens a device. The
:class:`ReplayTripletWorker` / :class:`LiveTripletWorker` below are the old
single-process in-process front-end drivers; they are kept for the window's
historical contract but their in-process acquisition graph
(``build_replay_frontend`` / ``build_live_frontend``) lives in the single-process
codebase, NOT in this device-free ``ui`` project, so invoking them here raises a
clear error. proc4 never reaches them (the IPC factory is injected).

cv2 / pyqtgraph are imported lazily (only when this window opens), so the base UI
stays lightweight; depthai is never imported (the UI is device-free by contract).
"""
...
class TripletWorker(threading.Thread):
    ...
    :class:`~ui.modules.triplet.UiTripletModule` sink. For each joined
    ...
    straight off the subscribed bus. ``error`` carries the first fatal reason
    ...
    def run(self) -> None:                                          # noqa: D401
        from ui.modules.triplet import UiTripletModule
        ...
        bus = LocalPubSub()
        try:
            self._drive(bus, UiTripletModule(bus, on_triplet))
...
class ReplayTripletWorker(TripletWorker):
    ...
    def _drive(self, bus, sink) -> None:
        # The in-process recorded-session front-end (``build_replay_frontend`` +
        # ``SessionReader``) lives in the single-process codebase, NOT in this
        # device-free ``ui`` project. proc4 never reaches this worker (it injects
        # the IPC adapter ``ipc_triplet_factory`` instead), so surface a clear
        # reason rather than a raw ImportError if it is ever invoked here.
        raise RuntimeError(
            "ReplayTripletWorker (in-process front-end) is not available in the "
            "device-free proc4 UI; inject ipc_triplet_factory(...) instead.")
...
class LiveTripletWorker(TripletWorker):
    ...
    (the single-process ``build_live_frontend``) off one shared device and
    subscribes ``frame.depth`` + ``imucam.sample``, so the triplet shown is
    literally the pipeline's output (no second device, no second SGM). It builds
    NO odometry/backend/slam -- the triplet needs none -- and runs the front-end
    ...
    def _drive(self, bus, sink) -> None:
        # The in-process live OAK-D front-end (``build_live_frontend``) lives in
        # the single-process codebase, NOT in this device-free ``ui`` project
        # (which never opens a device -- capture owns it). proc4 injects the IPC
        # adapter ``ipc_triplet_factory`` instead, so this worker is never reached
        # here; surface a clear reason rather than a raw ImportError if it is.
        raise RuntimeError(
            "LiveTripletWorker (in-process front-end) is not available in the "
            "device-free proc4 UI; inject ipc_triplet_factory(...) instead.")
...
class _DepthScaleBar(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        from ui.viz.depth_render import D_MAX, D_MIN, depth_scale_bar
...
class SyncedViewWindow(QWidget):
    ...
    def _show_depth(self, depth_m: np.ndarray) -> None:
        from ui.viz.depth_render import colorize_depth
