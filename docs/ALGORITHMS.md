...
and a separate `MapWindow` shows a *dense SLAM* cloud — neither shows per-window BA
...
> `MapWindow` (SLAM Map 3D room), `FloorPlanWindow` (Floor Plan top-down), and the two
> calib dialogs. `mainwindow.py`, `imucam_window.py`, and `viz/imucam_render.py` are
> legacy / not reachable from `ui/main.py`.
...
| **MapWindow (SLAM Map landmarks)** `ui/qt/map_window.py` + `ui/viz/map_cloud.py` (`longest_consecutive_run`, UI-only), fed by `IpcSlamMapSource` (`ui/modules/ipc_sources.py`) | Grid + ENU triad; the **sparse, ID-based landmark map** — ONE point per LANDMARK (KLT track id) that was a PnP INLIER across a run of ≥ `PERSIST_KF` (=20) **SUCCESSIVE keyframes** + amber keyframe-camera dots, re-snapped to SLAM's loop-corrected poses, in the SAME ENU frame as `Viewer3D`. Per keyframe `k`, each `inlier_ids` track's pixel → depth sample → pinhole back-project → world by `[R_keyframe \| t_corrected]`; a landmark kept only if the longest run of consecutive `k` it appeared in reaches `PERSIST_KF`, then output as the **median** of its world observations (mean gray). Strict, so a short run yields very few landmarks (tunable — LOWER `PERSIST_KF`). | **Confirmed-landmark map** of the keyframe/SLAM stage — only consistently-tracked, motion-validated points (NOT a dense reconstruction). Consumes vio `keyframe` (gray/depth/`track_ids`/`track_px`/`inlier_ids` via VIO's kf rings) + slam `slam.map` (corrected poses) — pure consumer, no data-path change. |
| **FloorPlanWindow (Floor Plan top-down)** `ui/qt/floor_plan_window.py` + `ui/viz/floor_plan.py` (`keyframes_to_ground_points` / `build_floor_plan`, pure numpy+cv2, UI-only), fed by `IpcFloorPlanSource` (`ui/modules/ipc_sources.py`) | A **LIGHT 2D top-down WALL raster** (NO `GLViewWidget`) — a readable alternative to the 3D map (heavy GL on a Mac, noisy depth hard to read in perspective). Each keyframe's depth is back-projected by its OWN VIO pose `[R \| t]` (strided, same depth gate + edge reject as the landmark map; **`MAX_DEPTH_M`≈4.0 m** — raised from 2.5 m so depth actually lands ON the far walls instead of only filling the near floor disc); the world-vertical optical-`+y` (down) axis is dropped to bin the points onto the optical `(x,z)` GROUND plane. It then renders the **WALL CELLS DIRECTLY**: a cell is a wall cell when its points span a tall vertical column (`extent_m = max_h − min_h ≥ WALL_EXTENT_M` — the *wall = high vertical extent* detector that excludes the flat floor) AND it has enough ray support (`MIN_CELL_COUNT`). Those cells **are** the walls (thin marks), cleaned by a *light* 2D `cv2` pass only (a tiny `MORPH_OPEN` drops 1-cell specks, `connectedComponentsWithStats` drops isolated noise blobs) — **NO `MORPH_CLOSE`, NO `MORPH_GRADIENT` outline**: outlining the occupied region just traces the camera's circular **sensing horizon**, not the walls, so drawing wall cells directly lets the room's true shape emerge (a square if the 4 walls were sensed; partial walls if not — honest). *Caveat (gold OAK-D stereo): fused per-cell vertical span is large nearly everywhere, so the gate cannot cleanly carve walls on that data; `WALL_EXTENT_M`≈2.0 m is tuned to be selective, and cleaner per-pixel depth (the VL53 ToF target) resolves walls properly.* Drawn bright over a faint raw-occupancy wash with the **camera path** + latest-pose marker on top. Rendered on a 2D `pyqtgraph.PlotWidget` (`ImageItem` + `PlotDataItem`), rebuilt off-thread at ≈5 Hz. Being a plain raster it can be written to a PNG with pure numpy/cv2 for offscreen visual verification. | **Top-down room layout** of the keyframe stage — walls as 2D marks + camera path, a light no-GL COMPLEMENT to the sparse landmark map. Consumes vio `keyframe` (depth via VIO's kf rings) only — pure consumer, no data-path change. Shares the `_KeyframeAccumulator` base (SHM/recv wiring) with `IpcSlamMapSource`. |
