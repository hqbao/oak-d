...
| **FloorPlanWindow (Floor Plan top-down)** `ui/qt/floor_plan_window.py` + `ui/viz/floor_plan.py` (`keyframes_to_ground_points` / `build_floor_plan`, pure numpy+cv2, UI-only), fed by `IpcFloorPlanSource` (`ui/modules/ipc_sources.py`) | A **LIGHT 2D top-down wall-outline raster** (NO `GLViewWidget`) — a readable alternative to the 3D map (heavy GL on a Mac, noisy depth hard to read in perspective). Each keyframe's depth is back-projected by its OWN VIO pose `[R \| t]` (strided, same depth gate + edge reject as the landmark map but a **tighter `MAX_DEPTH_M`≈2.5 m** because far stereo sprays radial fans top-down); the world-vertical optical-`+y` (down) axis is dropped to bin the points onto the optical `(x,z)` GROUND plane into an occupancy raster. A cell joins the room's **occupied region** only if hit by enough rays (`MIN_CELL_COUNT`) AND its points span a real vertical column (`FLOOR_EXTENT_M` — the explicit *wall = vertical extent* gate that drops the flat floor). That binary region is cleaned + reduced to a **crisp wall line** with cheap 2D `cv2` ops (`MORPH_OPEN` scrubs radial star-burst streaks, `MORPH_CLOSE` bridges depth-dropout gaps, `connectedComponentsWithStats` drops noise islands, `MORPH_GRADIENT` takes the region boundary), drawn bright over a faint raw-occupancy wash with the **camera path** + latest-pose marker on top. Rendered on a 2D `pyqtgraph.PlotWidget` (`ImageItem` + `PlotDataItem`), rebuilt off-thread at ≈5 Hz. Being a plain raster it can be written to a PNG with pure numpy/cv2 for offscreen visual verification. | **Top-down room layout** of the keyframe stage — walls as a 2D outline + camera path, a light no-GL COMPLEMENT to the sparse landmark map. Consumes vio `keyframe` (depth via VIO's kf rings) only — pure consumer, no data-path change. Shares the `_KeyframeAccumulator` base (SHM/recv wiring) with `IpcSlamMapSource`. |
