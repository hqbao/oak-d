...
(:mod:`ours.vio.loopclosure`) needs, so the ``ours-slam`` path carries no cv2
runtime dependency (matching what :mod:`ours.vio.klt` /
:mod:`ours.vio.corners` did for the VO frontend).
...
The final metric verification reuses our own :func:`ours.vio.pnp.solve_pnp_ransac`.
...
def match_ratio_mutual(desc_a: np.ndarray, desc_b: np.ndarray,
    ...
    :meth:`ours.vio.loopclosure.LoopDetector._good_matches`.
    """
